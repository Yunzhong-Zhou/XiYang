...
public class ContractDetailActivity extends BaseActivity {
    ...
    List<ContractDetailModel.WorkFlowApplylogOperateVoBean> list_shenhe = new ArrayList<>();
    CommonAdapter<ContractDetailModel.WorkFlowApplylogOperateVoBean> mAdapter_shenhe;
...
//        params.put("id", id);
    ...
    private void request(HashMap<String, String> params) {
        OkhttpUtil.okHttpGet(URLs.ContractDetail+id, params, headerMap, new CallBackUtil<ContractDetailModel>() {
            @Override
...
//                                .fitCenter()
                ...
                if (list_shenhe.size() > 0) {
                    showContentPage();
                    mAdapter_shenhe = new CommonAdapter<ContractDetailModel.WorkFlowApplylogOperateVoBean>
                            (ContractDetailActivity.this, R.layout.item_contractdetail_shenhe, list_shenhe) {
                        @Override
                        protected void convert(ViewHolder holder, ContractDetailModel.WorkFlowApplylogOperateVoBean model, int position) {
                            //隐藏最前和最后的竖线
