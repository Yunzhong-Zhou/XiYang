...
class RequestUtil {
    ...
    private RequestBody getRequestBody() {
        ...
        mParamsMap.put("_sign","");//签名参数，附带在url后面 ,此值由签名验证计算得到
        mParamsMap.put("_timestamp", System.currentTimeMillis()+"");//时间戳，毫秒， 此值应该为客户端时间戳+与服务端误差时间， 与服务端误差时间可以通过接口每次返回的serverTimestamp和本地时间戳进行计算
//        mParamsMap.put("_timestamp", "");
